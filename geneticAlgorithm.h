...
int crossover(int population[populationSize][numberOfAwards + 1]) {
	int oldArrayOfArrays[populationSize / 2][numberOfAwards + 1];
	for (int test = 0; test < populationSize / 2; test++) {
		overwriteArray(oldArrayOfArrays[test], population[test]);
	}
	int parrent2;
	for (int parrent1 = 0; parrent1 < populationSize / 2; parrent1++) {
		...
		for (int award = 0; award < numberOfAwards; award++) {
			if (rand() % 2 == 0) {
				population[parrent1 + populationSize / 2][award] = oldArrayOfArrays[parrent1][award];
				population[parrent1][award] = oldArrayOfArrays[parrent1][award];
			}
			else {
				population[parrent1 + populationSize / 2][award] = oldArrayOfArrays[parrent1][award];
				population[parrent1][award] = oldArrayOfArrays[parrent2][award];
			}
		}
	}
}
...
int geneticAlgorithm() {
	...
	for (int t = 0; t < numberOfTests; t++) {
		...
		int controlResult = 0;
		for (int p = populationSize / 2; p < populationSize; p++) {
			controlResult -= rivals[rivalsSize - 1][numberOfAwards];
		}
		fprintf(f, "control = %d\n\n", controlResult);
